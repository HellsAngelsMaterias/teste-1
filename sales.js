...
export const displaySalesHistory = (history, currentUser, currentUserData) => {
    // Se 'history' for null, usa o estado interno 'vendas'.
    ...
    // Visitantes só veem as próprias vendas
...
export const exportToCsv = () => {
    ...
    const csvRows = vendas.map(v => [`"${v.dataHora}"`, `"${venda.cliente}"`, `"${venda.organizacao}"`, `"${venda.organizacaoTipo}"`, `"${venda.telefone}"`, `"${venda.negociadoras}"`, `"${venda.vendaValorObs}"`, `"${venda.carro || ''}"`, `"${venda.placas || ''}"`, v.qtyTickets, v.qtyTablets, v.qtyNitro, v.valorTotal, `"${valorDescricao[venda.tipoValor]}"`, `"${venda.registradoPor}"`].join(','));

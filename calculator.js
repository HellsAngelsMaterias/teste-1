/* ===================================================
 * calculator.js
 * Responsável pela Calculadora, Registro de Vendas,
 * Histórico de Vendas, e auto-preenchimento.
 * =================================================== */

// --- IMPORTS ---
import { db, ref, set, push, onValue, remove, get } from './firebase.js';
import { els, showToast, formatCurrency, capitalizeText, toggleView } from './ui.js';
import { getCurrentUser, getCurrentUserData } from './auth.js';
// Importa funções do dossiê que serão usadas aqui
import { findDossierEntryGlobal, addDossierEntry, updateDossierEntryOnEdit } from './dossier.js';
...
export function loadVendas(vendasRef) {
    ...
    return vendasListener; // Retorna o listener para o auth.js poder cancelá-lo
}
...
// --- FUNÇÃO DE AUTO-PREENCHIMENTO (Ponte com Dossiê) ---
...
const autoFillFromDossier = async () => {
    ...
    try {
        // Usa a função importada do dossier.js
  ...
  operation
      .then(() => {
          ...
          if (dossierOrgDestino !== '') {
              // Usa as funções importadas do dossier.js
              if (vendaEmEdicaoId) {
...
export function initCalculator() {
    ...
    els.toggleHistoryBtn.onclick = () => {
        toggleView('history');
        displaySalesHistory(vendas); // Garante que o histórico é exibido ao abrir
    };
